...
Here is the complete, finalized report. I have seamlessly integrated the new **System Output and Data Generation Analysis** as Section 6.0, shifted the Conclusion to 7.0, and moved the User Manual to 8.0. 

This gives your assignment a perfect, logical flow from methodology, to performance results, to raw output proof, and finally the conclusion.
...
* **TEST A (Pure Sequential):** 1 Core, 1 File – Completed in 20.33 seconds.
* **TEST B (Parallel Only):** 16 Cores, 1 File – Completed in 16.45 seconds. This represents the fastest performance, showcasing the significant advantage of multi-core parallel processing for this cryptographic task.
* **TEST C (Concurrent Only):** 1 Core, 5 Files – Completed in 18.83 seconds. The results show that while concurrent writing was fast, it did not significantly reduce overall time, as the mathematical hashing was still performed sequentially on a single core.
...
*(Insert your screenshot of the text file inside Visual Studio here)* **Figure 2.0:** A sample snippet from the 'sequential_log_10M.txt' file.
